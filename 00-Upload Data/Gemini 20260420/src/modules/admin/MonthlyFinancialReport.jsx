...
// Version: 4.0 - Added Multi-Month Time & Labor Report feature
...
const MonthlyFinancialReport = () => {
    ...
    // --- NEW: Time & Labor Report State ---
    const [timeReportProject, setTimeReportProject] = useState(null);
    const [trStartMonth, setTrStartMonth] = useState('');
    ...
    // --- NEW: TIME & LABOR REPORT GENERATION ---
    const handleGenerateTimeReport = async () => {
        if (!trStartMonth || !trEndMonth) {
            alert("Please select both a start and end month.");
            return;
        }

        const startDate = new Date(trStartMonth + '-01');
        const endDate = new Date(trEndMonth + '-01');
        ...
        try {
            // 1. Build an array of report IDs to fetch (e.g., financial_summary_2024_01)
            const monthsToFetch = [];
            let curr = new Date(startDate);
            while (curr <= endDate) {
                const y = curr.getFullYear();
                const m = String(curr.getMonth() + 1).padStart(2, '0');
                monthsToFetch.push(`financial_summary_${y}_${m}`);
                curr.setMonth(curr.getMonth() + 1);
            }

            // 2. Fetch all required monthly reports and aggregate the data
            const employeeStats = {};
            let totalHours = 0;
            let totalCost = 0;
            let missingMonths = 0;

            for (const reportId of monthsToFetch) {
                const snap = await getDoc(doc(db, 'reports', reportId));
                if (snap.exists()) {
                    const data = snap.data();
                    if (data.byUser) {
                        data.byUser.forEach(user => {
                            // Find the project breakdown for the selected project
                            const pData = user.breakdown?.find(p => String(p.number) === String(timeReportProject));

                            if (pData) {
                                if (!employeeStats[user.name]) {
                                    employeeStats[user.name] = { hours: 0, cost: 0 };
                                }
                                employeeStats[user.name].hours += pData.hours;
                                employeeStats[user.name].cost += pData.cost;
                                totalHours += pData.hours;
            ...
            setTrData({
                ...
                totalCost,
                missingMonths,
                monthsScanned: monthsToFetch.length
            });

        } catch (err) {
            console.error("Failed to generate time report:", err);
    ...
    return (
        <div className="space-y-6">
                                                        ...
                                                        <div className="flex flex-col">
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <button
                                                                    onClick={(e) => {
                                                                        e.stopPropagation();
                                                                        setTimeReportProject(row.number);
                                                                        setTrData(null);
                                                                    }}
                                                                    className="p-1 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded"
                                                                    title="Time & Labor Report (Multi-Month)"
                                                                >
            ...
            {/* Time & Labor Report Modal */}
            <Modal show={!!timeReportProject} onClose={() => setTimeReportProject(null)} title={`Time & Labor Report: ${timeReportProject}`} maxWidth="sm:max-w-2xl">
                <div className="space-y-4">
                    <div className="flex items-end gap-4 bg-blue-50 p-4 rounded-lg border border-blue-100">
                        <div className="flex-1">
                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Start Month</label>
                            <input
                                type="month"
                                value={trStartMonth}
                                onChange={e => setTrStartMonth(e.target.value)}
                                className="w-full p-2 border rounded-md text-sm focus:ring-blue-500 focus:border-blue-500"
                            />
                        </div>
                        <div className="flex-1">
                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">End Month</label>
                            <input
                                type="month"
                                value={trEndMonth}
                        ...
                        <button
                            ...
                            disabled={trLoading || !trStartMonth || !trEndMonth}
                            className="px-4 py-2 bg-blue-600 text-white font-bold rounded-md hover:bg-blue-700 shadow-sm disabled:opacity-50"
                        >
                            {trLoading ? 'Fetching...' : 'Generate'}
                        </button>
                    </div>
                    ...
                    {trData ? (
                        <div className="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm">
                            <div className="p-3 bg-gray-50 flex justify-between items-center border-b border-gray-200">
                                <h3 className="font-bold text-gray-700 text-sm flex items-center">
                                    <UserGroupIcon className="h-4 w-4 mr-2 text-blue-600" /> Employee Breakdown
                                </h3>
                                <div className="text-xs text-gray-500">
                                    {trData.missingMonths > 0 && <span className="text-red-500 font-bold mr-2" title="Some months in your range have no cached financial report. Please generate them in the Monthly Reports section.">⚠️ {trData.missingMonths} months missing data</span>}
                                    <span className="font-medium bg-gray-200 px-2 py-0.5 rounded">{trData.monthsScanned} months scanned</span>
                                </div>
                            </div>
                            <div className="max-h-64 overflow-y-auto">
                                <table className="min-w-full text-sm">
                                    <thead className="bg-gray-100 sticky top-0 shadow-sm">
                                        <tr>
                                            <th className="px-4 py-2 text-left text-xs font-bold text-gray-500 uppercase">Employee</th>
                                            <th className="px-4 py-2 text-right text-xs font-bold text-gray-500 uppercase">Hours Logged</th>
                                            <th className="px-4 py-2 text-right text-xs font-bold text-gray-500 uppercase">Est. Labor Cost</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100">
                                        {trData.employees.length > 0 ? trData.employees.map((emp, i) => (
                                            <tr key={i} className="hover:bg-gray-50">
                                                <td className="px-4 py-2 font-medium text-gray-900">{emp.name}</td>
                                                <td className="px-4 py-2 text-right font-mono text-gray-600">{emp.hours.toFixed(2)}h</td>
                                                <td className="px-4 py-2 text-right font-mono text-gray-900">€{emp.cost.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                            </tr>
                                        )) : (
                                            <tr><td colSpan="3" className="px-4 py-8 text-center text-gray-400 italic">No hours found for this project in the selected range.</td></tr>
                                        )}
                                    </tbody>
                                    {trData.employees.length > 0 && (
                                        <tfoot className="bg-gray-50 sticky bottom-0 border-t-2 border-gray-200 font-bold">
                                            <tr>
                                                <td className="px-4 py-3 text-right text-gray-700 uppercase tracking-widest text-xs">Total:</td>
                                                <td className="px-4 py-3 text-right font-mono text-blue-700 text-base">{trData.totalHours.toFixed(2)}h</td>
                                                <td className="px-4 py-3 text-right font-mono text-orange-600 text-base">€{trData.totalCost.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                            </tr>
                                        </tfoot>
                                    )}
                                </table>
                        ...
                        <div className="py-12 text-center text-gray-400 border-2 border-dashed border-gray-200 rounded-lg">
                            <ClockIcon className="h-10 w-10 mx-auto mb-2 opacity-30 text-blue-500" />
                            <p className="text-sm font-medium">Select a date range to generate the report.</p>
                            <p className="text-xs mt-1 max-w-sm mx-auto">This tool uses your cached Monthly Financial Reports to provide incredibly fast, low-cost historical insights.</p>
                        </div>
                    )}
                </div>
            </Modal>
