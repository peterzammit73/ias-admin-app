// Root: src/App.jsx
// Version: 3.1 - Fixed Import Paths and set Default Landing to Task Board
...
import {
    ...
    XMarkIcon, CalculatorIcon, PaintBrushIcon
...
const MODULE_CONFIG = {
    ...
    admin: { name: 'Admin', slug: 'admin', icon: <ShieldCheckIcon />, defaultPath: 'dashboard', tabs: [{ label: 'Dashboard', path: 'dashboard' }, { label: 'Revenue Comparison', path: 'revenue-comparison' }, { label: 'Monthly Financials', path: 'monthly-financials' }, { label: 'Projects Report', path: 'projects-report' }, { label: 'Reports', path: 'reports' }, { label: 'User Permissions', path: 'permissions' }, { label: 'System Settings', path: 'settings' }] }
};
...
export default function App() {
    ...
    // Extracted the permission getter cleanly inside the component body
        ...
        <DataProvider>
            <Routes>
                ...
                <Route path="/" element={user ? <DashboardLayout userData={userData} handleSignOut={handleSignOut} /> : <Navigate to="/login" replace />}>
                    {/* Ensure everyone lands on the tasks page by default except admins who land on the dashboard */}
                    ...
                    <Route path="employees" element={<ProtectedRoute userData={userData} moduleKey="employees"><Outlet /></ProtectedRoute>}>
                        {/* Default route for the employees module is now tasks */}
